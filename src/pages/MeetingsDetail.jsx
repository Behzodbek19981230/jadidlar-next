...
export default function MeetingsDetail() {
  ...
  const fetchData = async () => {
    ...
    x.textContent = `Voqealar / Yig'inlar / ${response.title}`;
  };
  useEffect(() => {
    fetchData();
  }, []);
  ...
  return (
    <div className="jadidlar_container container_full">
      <Menu title="yiginlar_" link="/meetingsPage" subtitle={apiData?.title} />
      ...
      <div className="global_share">
        ...
        <a
          onClick={() =>
            (window.location.href = `https://telegram.me/share/url?url=http://jadidlar.uz/meetingsPage/${apiData?.id}`)
          }
          target="_blank"
        >
          <FaTelegramPlane />
